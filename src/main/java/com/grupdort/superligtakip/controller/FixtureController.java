...
public class FixtureController extends BaseController {
    ...
    private void renderFixtures(List<Fixture> fixtures) {
        ...
        for (Fixture f : fixtures) {
            ...
            matchCard.setSpacing(2); // Çok dar alan olduğu için boşlukları kıstık
            matchCard.setStyle("-fx-cursor: hand;");
            matchCard.setOnMouseClicked(event -> openMatchDetail(f.getFixture_ID()));

            // SAAT ALANI (SABİT MİNİK)
            String timeText = "";
            if (f.getMatch_time() != null) {
                if (!isAllSunday) timeText = f.getMatch_time().format(timeFormatter);
                if (f.getMatch_time().getHour() == 0 && f.getMatch_time().getMinute() == 0) timeText = "";
            }

            Label timeLbl = new Label(timeText);
            timeLbl.getStyleClass().add("match-time");
            // 400px için saati 35px ile sınırladık
            timeLbl.setMinWidth(35);
            timeLbl.setPrefWidth(35);
            timeLbl.setMaxWidth(35);
            timeLbl.setAlignment(Pos.CENTER);

            // EV SAHİBİ
            HBox homeBox = createTeamBox(f.getHome_team().getTeam_name(), f.getHome_team().getLogo(), true);
            // Boşluğu doldurması için Priority.ALWAYS veriyoruz
            HBox.setHgrow(homeBox, Priority.ALWAYS);
            // İkisi de eşit "start" noktasından büyüsün diye prefWidth 1 veriyoruz
            ...
            // Skoru 50px'e sabitledik
            scoreLbl.setMinWidth(50);
            scoreLbl.setPrefWidth(50);
            scoreLbl.setMaxWidth(50);
            ...
            awayBox.setPrefWidth(1); // Eşit dağılım için
            awayBox.setMaxWidth(Double.MAX_VALUE);

            // YILDIZ
            Button starBtn = createStarButton();
            starBtn.setMinWidth(25); // Buton genişliği
            starBtn.setMaxWidth(25);
    ...
    private HBox createTeamBox(String name, String logoUrl, boolean isHome) {
        ...
        // Taşma durumunda label minimum 0'a iniyor
